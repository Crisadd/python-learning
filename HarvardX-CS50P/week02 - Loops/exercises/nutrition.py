...
raw fruits … in the United States. Retail stores are welcome to download the posters, print, display and/or distribute them to consumers in close proximity to the
...
that fruit, per the FDA’s poster for fruits, which is also available as text. Capitalization aside, assume that users will input fruits exactly as written in the poster
...
"""
check_calories = input('Item: ').title()
...
for key, value in fruit_calories.items():
    if key == check_calories:
